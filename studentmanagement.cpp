...
void load_all_data() {
    ...
    while (getline(allfile, id)) {
        ...
        while (getline(file, data)) {
            ...
            if (d == 1) allStudentList[id].ID = temp;
            else if (d == 2) allStudentList[id].name = temp;
            else if (d == 3) allStudentList[id].academicYr = n;
            else if (d == 4) allStudentList[id].stdClass = n;
            else if (d == 5) allStudentList[id].rollNo = n;
            else if (d == 6) allStudentList[id].age = n;
            else if (d == 7) allStudentList[id].fathers_name = temp;
            else if (d == 8) allStudentList[id].mothers_name = temp;
            temp.clear();
            d += 1;
        }
    }
...
void updateStudent() {
    ...
    cin.ignore();
    getline(cin, temp);
    if(temp.size()!=0) std.name=temp;
    temp.clear();
    cout << "Enter student Father's name : ";
    // cin.ignore();
    getline(cin, temp);
    if(temp.size()!=0) std.fathers_name=temp;
    temp.clear();
    cout << "Enter student mother's name : ";
    // cin.ignore();
    getline(cin, temp);
    if(temp.size()!=0) std.mothers_name=temp;
    temp.clear();
...
int main() {
    struct stat sb; 
    if (stat("StudentInfo", &sb) != 0) mkdir("StudentInfo");
